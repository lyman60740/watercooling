...
function TubeWithLiquid({ curve }) {
  ...
  useFrame(() => {
    ...
    const points = curve.getSpacedPoints(1000).slice(0, Math.floor(1000 * progress));
    
    if (points.length > 1 && liquidRef.current) {
      const tubeGeometry = new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points), 100, 0.031, 20, false);
...
function App() {
  ...
  return (
    <div className="App">
      <Canvas className="canvas" style={{ background: 'black' }} camera={{
        position: [0, 0, 1.8],
        fov: 75,
        near: 0.1,
        far: 1000,
        ...
        <directionalLight
          ...
          intensity={2}
        />
        <directionalLight position={[0 + mousePosition.x * 0.1, 3 + mousePosition.y * 0.1, 2]} intensity={2} />
